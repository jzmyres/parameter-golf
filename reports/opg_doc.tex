...
\emph{composable} collapse-prevention auxiliaries, each an independent CLI knob defaulting to off so
the no-auxiliary fixed-\(L_1\) run is the control.  All three are differentiable router-only
regularizers (they train the router on the saved block input); the forward routing map is unchanged,
so reversibility is unaffected.
\begin{enumerate}
