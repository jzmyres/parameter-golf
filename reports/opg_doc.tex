...
recurrence helps) plus an eval-depth \(\varphi\) proxy fitted over the \(\{K:\text{loss}(K)\}\) map.
The sweep is \emph{paired so depth \(K\) is the only variable}: the evaluation batches are
materialized once and replayed for every \(K\) (the streaming loader would otherwise score each \(K\)
on different samples), and the RNG state is captured once and reset before each \(K\) so that under
\code{--init-state random} every \(K\) draws the same recurrence-seed noise.  Without both, \(G_T\)
and the \(\varphi\) proxy conflate depth with sample and seed variance; with both, the depth-gain is
a reproducible controlled measurement on fixed weights, and the captured RNG is restored afterward so
the artifact-save path is unaffected.  The \(\varphi\) fit drops non-finite depths so a single
