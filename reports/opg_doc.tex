...
recurrent depth \(K\) after the final validation (rank-0, end-of-run only) and emits
\(G_T=\bpb(K_{\min})-\bpb(K_{\max})\) (positive when deeper recurrence helps) plus an eval-depth
\(\varphi\) proxy fitted over the \(\{K:\text{loss}(K)\}\) map.  When collapse is detected, the
