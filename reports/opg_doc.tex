...
By default the sweep runs \emph{once} after the final validation, but the opt-in \code{--depth-sweep-every}
\(N\) flag additionally runs the \emph{same} depth/expressiveness battery (\code{depth\_sweep},
\(G_T\)/\code{depth\_gain\_GT}, \code{phi\_eval}, \code{usable\_info}, \(I_{\mathcal V}\)/\code{expressiveness\_rho},
\code{iv\_total\_bits}) every \(N\) validation-aligned steps, with each line step-prefixed
(\code{step:\textit{s} depth\_gain\_GT:\(\cdots\)}), so the expressiveness battery (\(\varphi\), \(I_{\mathcal V}\),
\code{expressiveness\_rho}, \(G_T\)) has a trackable \emph{trend} over training rather than only an
end-of-run value---the smoke-gate ``all eval metrics improving'' needs the \(\varphi\)/\(I_{\mathcal V}\)
trends, not just the per-step \code{metrics:} line. Default \(N=0\) preserves the byte-identical
end-of-run-only behavior; the final unprefixed sweep is always emitted.

\emph{Test-time depth extrapolation.}  The \code{--k-eval-sweep} should include depths \emph{beyond}
the trained maximum \(K\) (e.g.\ train on \(K\in\{32,64\}\), sweep \(\{4,8,16,32,64,128,256\}\)) so that
\(I_{\mathcal V}(K)\) and \(\bpb(K)\) are measured in the extrapolation regime.  The reading is
diagnostic: if \(I_{\mathcal V}\) keeps rising / \(\bpb\) keeps falling past the trained horizon, the
recurrence exhibits Huginn-style test-time depth scaling \cite{huginn}; if it degrades (the tied,
finite-horizon, non-nested readout \emph{over-thinks} beyond the trained \(K\), \S\ref{par:vinfo}), the
extrapolation cliff is itself the signal and bounds the usable inference depth.  Either way the
extrapolation tail is tracked at validation, never assumed.
...
\emph{Depth-utility is data-determined; ``shallow vs.\ deep'' is read from the curve, not a point.}
A single \(I_{\mathcal V}(K)\) value cannot say whether a model solves a task shallowly or deeply --- only
the \emph{curve} \(K\mapsto I_{\mathcal V}(K)\) can, and \code{iv\_total\_bits}
\(=I_{\mathcal V}(K_{\max})-I_{\mathcal V}(K_{\min})\) \emph{is} the depth-contribution (the slice of usable
information that depth, not the shallow pass, supplies).  On easy LM next-token prediction the model gets
most of its predictive information \emph{shallowly}: measured on fineweb, \(\approx79\%\) of the depth-32
usable information is already realized at \(K{=}4\), and the remaining depth adds only \(\approx0.79\)
bits.  Consequently \(\varphi\) / depth-utility is intrinsically \emph{modest on easy LM} and is large only
on depth-demanding data --- the provably depth-hard \code{experiments/p1\_synthetic.py} protocol
(\S\ref{sec:protocol}).  The \emph{magnitude} of \(\varphi\) and \code{iv\_total\_bits} is
therefore data-determined, not a model failure: a flat \(I_{\mathcal V}\) curve on easy text is expected and
is not evidence of recurrence collapse (which is diagnosed against the positive control, not against the
easy-LM curve).
...
\paragraph{Training recipe (LR is a stability constraint, not a free knob).}
The optimizer is Muon (matrix banks, including the 3-D MoE expert weights) plus AdamW (embeddings,
routers/gates, 1-D scalars), with \(\beta_2{=}0.95\) and gradient clipping at \(1.0\) --- both matching
Huginn \cite{huginn}.  Deep-horizon recurrent-depth training is \emph{LR-sensitive}: the previous peak
LRs (matrix \(0.02\), embed \(0.1\), scalar \(0.02\)) \emph{diverge} (train loss \(6.9{\to}16.5\),
hidden-state collapse).  The promoted defaults \emph{halve} them (matrix \(0.01\), embed \(0.05\),
scalar \(0.01\)), which restores a monotone-decreasing train loss, drops \(\bpb\) \(3.68{\to}2.03\),
moves \(I_{\mathcal V}\) from \(-0.2\) to \(+3.7\) bits, and keeps routing healthy
(\(\code{\detokenize{expert_util}}\approx2.7\)).  This mirrors Huginn, which explicitly reduced its peak
LR to prevent exactly this recurrent hidden-state collapse \cite{huginn}: in a tied recurrence the same
weights are applied \(K\) times per forward, so an LR that is fine for a single-application block is
effectively \(K\)\(\times\) too aggressive on the recurrent state.

\paragraph{Consolidated configuration (source of truth: \code{train\_gpt.py}).}
Table~1 collects the active \(M_0\) architecture and training defaults in one place;
the binding source is \code{Hyperparameters} plus \code{build\_arg\_parser} in \code{train\_gpt.py}
(this table mirrors but does not own them).  At these defaults the model is \(9{,}666{,}338\)
parameters.  The architecture (left) and the optimization/finite-horizon recipe (right) map directly
onto the two design goals: the resource-side knobs (MLA \code{\detokenize{kv_latent}}, low-rank
experts, the fp64 reversible stream) hold activation memory and the KV footprint small, while the
expressiveness-side knobs (recurrent depth \code{\detokenize{k_set}}, the MoE basis, the MoS head)
supply usable predictive information.

\begin{center}
\footnotesize
\renewcommand{\arraystretch}{1.12}
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{@{}L{0.27\textwidth} l @{\hskip 1.2em} L{0.27\textwidth} l@{}}
\toprule
\multicolumn{2}{@{}l}{\textbf{Architecture}} & \multicolumn{2}{l}{\textbf{Training / finite-horizon recipe}} \\
\midrule
\(d_{\rm model}\) (\code{\detokenize{model_dim}})        & 768   & Optimizer (matrix banks)   & Muon \\
Query heads (\code{\detokenize{n_heads}})                & 12    & Optimizer (embed/router/1-D) & AdamW \\
KV heads (\code{\detokenize{n_kv_heads}}, GQA)           & 4     & \code{\detokenize{matrix_lr}} & \(0.01\) \\
Head dim (\code{\detokenize{head_dim}})                  & 64    & \code{\detokenize{embed_lr}}  & \(0.05\) \\
MLA KV-latent (\code{\detokenize{kv_latent}})            & 256   & \code{\detokenize{scalar_lr}} & \(0.01\) \\
Vocab (\code{\detokenize{vocab_size}})                   & 1024  & \(\beta_2\) / grad-clip       & \(0.95\) / \(1.0\) \\
Routed experts (\code{\detokenize{n_experts}})           & 16    & Deep set \code{\detokenize{k_set}} (\(K_{\rm hi}\)) & \(\{32,64,128\}\) \\
Expert rank (\code{\detokenize{expert_rank}})            & 64    & Shallow \(k_{\rm lo}\)        & \(8\) \\
Shared experts (\code{\detokenize{num_shared_experts}})  & 0     & Hinge weight \(\lambda_h\)    & \(0.1\) \\
Sublayers (\code{\detokenize{n_sublayers}})              & 1     & Hinge margin \(m\)            & \(0\) \\
Block order (\code{\detokenize{block_order}})            & \code{attn\_ffn} & Train context (\code{\detokenize{seq_len}}) & \(512\) \\
Router (\code{\detokenize{router_type}})                 & \code{softmax}   & Recurrence stream (\code{\detokenize{recurrence_accum_dtype}}) & \code{float64} \\
MoS components (\code{\detokenize{n_mix}})               & 2     & Init state (\code{\detokenize{init_state}}) & \code{x0} \\
Reversible blocks                                        & 1 (tied over \(K\)) & Damping (\code{--damping})   & \code{none} (\(Q{=}I\)) \\
\bottomrule
\end{tabularx}

\smallskip
\emph{Table~1: active \(M_0\) configuration (\code{train\_gpt.py} defaults; \(9{,}666{,}338\)
parameters).  Router health defaults: DeepSeek loss-free balancing
\code{\detokenize{router_bias_update_rate}}\(=10^{-3}\) and ST-MoE
\code{\detokenize{router_z_coef}}\(=10^{-3}\); entropy-max and load-balance off.}
\end{center}
...
\end{equation}
\emph{Interpretation.}  This is the reversible round-trip error of the additive-coupling reversible
architecture lineage~\cite{revnet,reformer,revffn}, on which exact reconstruction rests.
The \(O(1)\)-in-\(K\) reversible backward (\code{RevRecurrenceFn}) does not
...
three finite \(K\) (no rank correlation estimable).  This is the headline expressiveness reading; the
ranked quantity is the \(\mathcal V\)-usable predictive information~\cite{vinfo} of
Eq.~\eqref{eq:iv-metric}; gate-direction \(\to+1\).
...
\(\mathcal V\)-usable information~\cite{vinfo} depth buys across the swept range.

\paragraph{\(G_T\) --- depth-gain (\GATE\ summary).}
\(G_T(K_{\rm lo},K_{\rm hi})=\bpb(K_{\min})-\bpb(K_{\max})\) (bits-per-byte units), the two-point bpb
summary of \(I_{\mathcal V}\) and the operational P1 statistic; it operationalizes the recurrent-depth
and iso-depth scaling framing~\cite{huginn,isodepth}.  \emph{Gate rule:} paired-CI lower
...
task tolerance \(\epsilon\); a guard that depth must not hurt --- the test-time depth-scaling
monotonicity property~\cite{huginn} --- with rule \(\mathrm{NDR}_\epsilon\le
\delta_{\rm task}\).
...
\code{--init-state random} and \code{--damping orthogonal} fixes target).  This is our own diagnostic
for autonomous depth signal, related to the effective-depth notion of the iso-depth scaling
analysis~\cite{isodepth}.
...
every step (MoE-basis unused, the across-depth collapse mode) --- the looped-MoE diversity-recovery
target of~\cite{moeut,sparseloopedmoe}.
...
\(\Rightarrow\) diverse expert functions, \(\approx0\Rightarrow\) duplicated/degenerate experts (the
basis-degeneracy failure the looped-MoE works~\cite{moeut,sparseloopedmoe} guard against).
...
routing.  This is specialization, \emph{not} utilization, and tracks the router stability that the
load-balancing~\cite{switch} and z-loss~\cite{stmoe} regularizers shape.
...
\(\ln E\) \(\Rightarrow\) all experts used, low \(\Rightarrow\) dead experts --- the balancing objective
of the load-balance auxiliary~\cite{switch} and its auxiliary-loss-free variant~\cite{deepseekv3}.
\(\code{\detokenize{diag:active_frac}}\)
...
flat in recurrent depth (the activation-memory-saving property of reversible
architectures~\cite{revnet,reformer,revffn,melt}).  Dropped as resource metrics: the artifact-16\,MB hard gate (artifact bytes
...
reversible recurrence's defining \(O(1)\)-in-\(K\) activation-memory property~\cite{revnet,reformer,revffn,melt}.  \GATE\ (P2).
...
\(\approx\)constant in recurrent depth, the P2 claim and the reversible \(O(1)\)-in-\(K\) backward's
defining benefit~\cite{revnet,reformer,revffn,melt}.  Supplied by the control sweep.  \GATE\ (P2).
...
which the \(O(1)\)-in-\(K\) reversible recurrence~\cite{revnet,reformer,revffn,melt} makes affordable at deep \(K\).

\paragraph{\(\code{\detokenize{kv_bytes}}\) --- MLA KV-cache footprint (\DIAG).}
Bytes per token of the MLA-compressed KV cache~\cite{deepseekv2} (the latent plus decoupled RoPE channel, billed bf16);
...
\emph{Empirically shelved (negative result, 2026-06).}  Orthogonal / mHC damping
(\code{--damping orthogonal}, \(Q=\expm(S)\)) is implemented but stays \emph{default off and is not
promoted}.  Across two clean attempts it did \emph{not} raise across-step path diversity
\(\code{\detokenize{route_step_div}}\) (the MoE basis stayed unengaged at \(\approx0.04\)) \emph{and} it
destabilized the bf16 reconstruction (\(\code{\detokenize{recon_rel}}\to0.85\), driven by the \(F/G\)
state dynamic range, which would require explicit state-norm control to tame).  The conclusion is that
depth-utility arises from the recurrence \emph{dynamics} --- the additive coupling with correct fp64
gradients --- not from MoE-basis exponential mixing paths; \code{\detokenize{route_step_div}},
velocity-state routing, and damping were all explored and none drove the across-step diversity.  The MoE
is therefore retained as the parameter-\emph{efficiency} basis (\(\varphi_{\rm isodepth}\),
\S\ref{ssec:efficiency}), not as the depth-utility mechanism.
...
%==============================================================================
\section{\texorpdfstring{$M_0$}{M0} Empirical Results: Depth-Monotone Expressiveness Under Flat Resource (2026-06-06)}
\label{sec:m0-results}

This section reports a controlled run of the \emph{active} \(M_0\) language model (\code{train\_gpt.py},
the architecture of \S\ref{sec:models} at the Table~1 defaults), distinct from the synthetic S+0 P1
triage above.  Its purpose is twofold: (i) verify that \emph{every} two-goal eval metric improves as
training progresses, and (ii) characterize how each metric responds to recurrence depth \(K\) at the
final checkpoint, \emph{including} the test-time extrapolation regime beyond the trained horizon.
The run is the half-LR DDP smoke that verified the promoted (halved) LR defaults (\S Training recipe);
the frozen evidence is \code{results/2026-06-06\_m0\_halflr\_extrapolation/} (\code{summary.md} and
\code{run\_metrics.txt}).

\paragraph{Setup.}
Two GPUs under \code{torchrun} DDP (\code{\detokenize{nproc_per_node=2}}), \(600\) optimizer steps,
\(d_{\rm model}=768\) and the Table~1 architecture, \(9{,}666{,}338\) parameters.  Finite-horizon
training used \code{\detokenize{k_set}}\(=\{32,64\}\) (\(K_{\rm hi}\) sampled per step) with shallow
\(k_{\rm lo}=8\); the promoted LRs (matrix \(0.01\), embed \(0.05\), scalar \(0.01\)); \code{--damping
none} (\(Q{=}I\)); \code{\detokenize{init_state}}\(=\)\code{x0}; and the fp64 reversible stream
(\code{\detokenize{recurrence_accum_dtype}}\(=\)\code{float64}).  The end-of-training depth sweep
\code{--k-eval-sweep 4,8,16,32,64,128,256} deliberately includes \(K\in\{128,256\}\), which lie
\emph{beyond} the trained maximum \(K{=}64\), so \(I_{\mathcal V}(K)\) and \(\bpb(K)\) are measured in
the extrapolation regime (\S\ref{sec:protocol}); \code{--depth-sweep-every 200} additionally tracked the
expressiveness battery as a training trend.

\paragraph{(i) All two-goal eval metrics improve over training.}
Table~2 reports the expressiveness, correctness, and resource metrics at the first tracked sweep
(step 200) versus the final checkpoint (step 600 / end-of-run).  Every expressiveness metric rises and
\(\bpb\) falls monotonically, while the correctness gate stays exact (\code{\detokenize{recon_rel}}\(=0\))
and the resource number stays flat --- the conjunction the 30-iter smoke gate (\S\ref{sec:protocol})
demands.
...
\begin{tabularx}{\textwidth}{@{}L{0.40\textwidth} r r L{0.18\textwidth}@{}}
\toprule
\textbf{Metric (axis)} & \textbf{step 200} & \textbf{final} & \textbf{Trend} \\
\midrule
\(\bpb\) (expressiveness, \(\downarrow\))                                 & \(2.131\)  & \(\mathbf{1.828}\) & monotone \(\downarrow\) \\
\(I_{\mathcal V}(K{=}64)\) bits (expressiveness, \(\uparrow\))            & \(3.494\)  & \(\mathbf{4.178}\) & rising \\
\code{\detokenize{iv_total_bits}} (\(K{=}4{\to}256\)) (expressiveness, \(\uparrow\)) & \(0.619\) & \(\mathbf{0.797}\) & rising \\
\code{\detokenize{depth_gain_GT}} (expressiveness, \(\uparrow\))          & \(0.258\)  & \(\mathbf{0.324}\) & rising \\
\code{\detokenize{phi_eval}} (efficiency proxy, \(\uparrow\))             & \(0.095\)  & \(\mathbf{0.116}\) & rising \\
\code{\detokenize{recon_rel}} (correctness gate, \(\to0\))               & \(0\)      & \(\mathbf{0}\)     & exact throughout \\
\code{\detokenize{peak_vram}} MB (resource, flat in \(K\))               & \(\approx30{,}468\) & \(\approx30{,}468\) & flat \\
\code{\detokenize{expert_util}} (MoE health)                             & \(\approx2.70\) & \(\approx2.70\) & healthy \\
\code{\detokenize{erank}} (representation diag.)                         & \(636\)    & \(550\)            & healthy (non-degenerate) \\
\bottomrule
\end{tabularx}

\smallskip
\emph{Table~2: two-goal metrics over training (step 200 \(\to\) final), 2026-06-06 \(M_0\) half-LR
run.  \code{\detokenize{erank}} is an explicitly non-monotone diagnostic (\S\ref{ssec:expressiveness}),
so its mild decline is not a regression.}
\end{center}

\paragraph{(ii) Depth strictly helps within the trained horizon; the finite-horizon cap shows beyond it.}
Table~3 is the final unprefixed \code{--k-eval-sweep}.  Within the trained horizon \(K\le64\) the
recurrence is \emph{strictly depth-monotone}: \(\bpb(K)\) decreases and \(I_{\mathcal V}(K)\) increases
at every step from \(K{=}4\) to the trained maximum \(K{=}64\), where both peak.  Beyond the horizon
(\(K\in\{128,256\}\), extrapolation) both metrics \emph{gently reverse}.  This is the expected
finite-horizon signature of a tied, non-nested readout (\S\ref{par:vinfo}: deep \(K\) can over-think
past the trained budget), \emph{not} a training regression --- the model was never trained to use depth
beyond \(64\).
...
\toprule
\(K\) & \(4\) & \(8\) & \(16\) & \(32\) & \(\mathbf{64}\) & \(128\) & \(256\) \\
\midrule
\(\bpb(K)\)              & \(2.2129\) & \(1.9842\) & \(1.8721\) & \(1.8269\) & \(\mathbf{1.8186}\) & \(1.8379\) & \(1.8885\) \\
\(I_{\mathcal V}(K)\) bits & \(3.20\)   & \(3.76\)   & \(4.04\)   & \(4.15\)   & \(\mathbf{4.17}\)   & \(4.12\)   & \(4.00\) \\
regime                  & \multicolumn{5}{c}{\hrulefill\ trained \(K\le64\) (strictly monotone)\ \hrulefill} & \multicolumn{2}{c}{extrapolation \(\downarrow\)} \\
\bottomrule
\end{tabular}

\smallskip
\emph{Table~3: final per-depth validation (bold = trained maximum \(K{=}64\), the best depth).
\(\bpb\) strictly decreases and \(I_{\mathcal V}\) strictly increases for \(K{=}4{\to}64\); both reverse
beyond the trained horizon.}
\end{center}

\paragraph{Reading \code{\detokenize{expressiveness_rho}} honestly under finite-horizon design.}
The full-range Spearman correlation over the swept \(K\) is
\(\code{\detokenize{expressiveness_rho}}=0.5357\), \emph{not} \(+1\).  This is by design, not a failure:
the headline gate (Eq.~\eqref{eq:expr-rho}) is \(\rho_{\rm Spearman}(K,I_{\mathcal V}(K))\) over the
\emph{entire} sweep, which here includes the post-\(K{=}64\) extrapolation dip, so a finite-horizon model
\emph{cannot} reach \(\rho=+1\) on a range that extends past its trained depth.  Restricted to the
in-range depths \(K\le64\) the curve is monotone and \(\rho\approx1.0\).  The single ``flat'' headline
number is therefore the finite-horizon extrapolation cap (\S\ref{par:vinfo}), and consuming deeper \(K\)
at test time would require either training deeper (extend \code{\detokenize{k_set}}) or an anytime /
convergent readout (\S\ref{par:vinfo}), a separate design axis.

\paragraph{Two-goal reading.}
This run substantiates the two-goal frame (\S\ref{ssec:two-goal}) on the active model.  \emph{Resource:}
the reversible \(O(1)\)-in-\(K\) backward holds \code{\detokenize{peak_vram}} flat across the depth sweep
and the fp64 stream keeps \(\code{\detokenize{recon_rel}}=0\) (gradient correctness), so deeper inference
costs no activation memory and the loop param count is meaningful.  \emph{Expressiveness:} within the
trained horizon, each added recurrence step buys \(\mathcal V\)-usable predictive information
(\(I_{\mathcal V}\,\uparrow\), \(\bpb\,\downarrow\)).  The two goals hold \emph{simultaneously}:
depth-monotone usable information at flat, bit-exact resource is exactly the Pareto behavior the design
targets.  The modest absolute depth-contribution (\code{\detokenize{iv_total_bits}}\(\approx0.8\) bits)
is the data-determined easy-LM regime (\S\ref{par:vinfo}); the depth-hard \code{p1\_synthetic} protocol
remains the venue for large \(\varphi\) / depth-utility.
