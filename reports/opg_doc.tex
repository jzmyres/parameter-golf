...
inverse, so gradients equal ordinary BPTT only if the round-trip is exact: \(\approx0\) at machine
precision in fp64 (proven by the reconstruction unit tests), a small nonzero in bf16 (additive
non-associativity over \(K\) steps).  A large \(\code{\detokenize{recon_rel}}\) means the
reconstructed graph --- and hence the gradient signal --- drift from the true forward pass.  It is a
