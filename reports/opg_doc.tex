...
\begin{enumerate}[leftmargin=1.6em,itemsep=1pt]
  ...
  \code{step\_conditioning}, off by default), and \(Q\) is the optional orthogonal (low-rank
  skew-Cayley) damping (\code{--damping}, default \code{none}\(\Rightarrow Q{=}I\)): strictly invertible
  (\(Q^{\top}\)), norm-preserving (no fixed-point collapse, extrapolates beyond the trained depth), and
  relaxing the \(A{+}B{=}I\) convex tie.
