...
the premise of the first experiment.  Therefore the final minimal core deliberately removes feature
stacks that obscure P1 failure attribution: dense MoE, Dirichlet--UCB routing, MLA, MoS,
quantization, implicit/DEQ-style backward, fixed-point consistency, spectral or Lyapunov pressure,
UFID, distillation, and anytime losses are excluded from the P1 science path.  The executable
...
\paragraph{Core \(M_0\).}
The core model is a tied, token-injected, additive-coupling reversible recurrence with standard MHA/GQA plus SwiGLU and a halting readout.  The recurrent state is
...
The readout uses a halting gate \(g_k=\sigma(w^\top z_k)\), where \(z_k\) denotes the readable
state derived from \(s_k\).  For a recurrent budget \(K\):
\begin{equation}
  o_K
  =
  \sum_{k=1}^{K}
  \left(
    g_k\prod_{j<k}(1-g_j)
  \right)z_k .
  \label{eq:halting-readout}
\end{equation}
If \(g_{K_{\rm lo}}=1\), the depth-\(K_{\rm lo}\) output is reproducible inside the
depth-\(K_{\rm hi}\) hypothesis class for \(K_{\rm hi}>K_{\rm lo}\).  This gives the desired
nestedness:
\[
  \mathcal F_{K_{\rm lo}}\subseteq\mathcal F_{K_{\rm hi}},
\]
and hence achievable risk monotonicity.  The trained model can still fail; the point is that the
architecture no longer makes deeper inference intrinsically non-nested.
...
The following are not part of the core science path: Dirichlet--UCB router, MLA, mixture-of-softmaxes
head, int6 quantization, DEQ or implicit backward, prefix-anchor consistency, spectral or Lyapunov
pressure, UFID, distillation, and anytime losses.  They may remain in the repository as legacy,
deployment, or later-stage axes, but they should not be used to establish P1.
...
\(\alpha_{\rm act}\), \(R_{\rm rec}\), \(\vram\) & \GATE\ (P2) & \(R_{\rm act}(K,2K)\le1.10\); reconstruction error acceptable under actual training stochasticity; \(\vram\le B\), reported per component. \\
...
A4 & Halting gate can saturate & Verify \(g_k\to1\) is reachable; otherwise nestedness is only approximate. \\
