...
| entmax_blend_lr | 0.02 (iter 117b-1: 0.002 → 0.02, **10× bump** to give the optimizer authority to drive `_entmax_blend_logit` away from init=5.0 (sigmoid≈0.99, near-pure-softmax) toward smaller values that activate the entmax contribution. Per H87 RESULT, the previous 0.002 was too slow to move the blend over a 1000-step horizon. Strict-gen at init preserved (logit=5 still recovers iter 100b within bf16 floor at step 0); the LR change only affects the rate at which the blend can DRIFT toward entmax if val_bpb gradient says it helps.) |
| muon_momentum | 0.99 |
...
| router_entropy_coef | 0.05 (iter 117b-1: 0.005 → 0.05, **10× bump** from H87 RESULT observation that pertoken_entropy stuck at 2.80 in iter 117 v5; previous magnitude was dwarfed 25× by cv contribution. Per-token entropy MINIMIZATION term `+coef·H_pertoken` added to total loss → drives `H_pertoken → 0` → per-token specialization. CV loss handles dead-expert prevention orthogonally; the two regs operate on different axes — per-token sparsity vs global balance — and do NOT antagonize.) |
