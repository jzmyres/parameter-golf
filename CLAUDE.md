...
- **Routing regularizers operate on routed mass.** Router regularizers use combined `p = simplex(allocation_scores) * sigmoid(gate)` so load balance sees gate effects. MoS softmax is the exception. Details: [`EXPERIENCE.md#routing-reg-input-invariant`](EXPERIENCE.md#routing-reg-input-invariant).
