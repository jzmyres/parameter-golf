...
| router_entropy_coef | 0.005 (iter 100b: per-token entropy MINIMIZATION term `+coef·H_pertoken` added to total loss → drives `H_pertoken → 0` → per-token specialization. CV loss handles dead-expert prevention orthogonally; the two regs operate on different axes — per-token sparsity vs global balance — and do NOT antagonize. If `pertoken_entropy` stays high in training, the issue is magnitude, not sign — bump this coef.) |
