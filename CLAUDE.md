...
- **Project-doc results-update mechanism.** When a run improves the eval metrics, refresh the empirical results in [`reports/opg_doc.tex`](reports/opg_doc.tex): (1) an **over-training trend table** showing ALL eval metrics improving as training progresses (`val_bpb`↓, `I_V`/`phi_eval`/`depth_gain_GT`/`iv_total_bits`↑, `recon_rel`=0, resource flat), and (2) a **final per-depth-K validation table** showing recurrence depth `K` strictly improves the metrics within the trained horizon (`val_bpb`↓, `I_V`↑ for `K` up to `K_hi`), reported honestly through the test-time extrapolation tail (the finite-horizon cap beyond `K_hi` is a result, not a regression). Freeze the run's evidence in `results/<date>_<tag>/` (`summary.md` + `run_metrics.txt`) and cite it from the doc; mirror the verdict in `experiments/docs/hypotheses.md`. See `results/2026-06-06_m0_halflr_extrapolation/` for the template.
...
Orientation mirror only — `train_gpt.py` `Hyperparameters` + `build_arg_parser` defaults are the source of truth; check them before launch. The implementation **is** the minimal design in `reports/opg_doc.tex` (reversible recurrent depth + MLA + MoE + MoS), not the archived rich RevDEQ model (see `legacy/train_gpt_rich.py`). The legacy rich knobs (Dirichlet-UCB, Parcae damping, `experts_per_slot`/`expert_slots`, `use_ctp`/`lyapunov`/`num_refinements`, profiles) do **not** exist in M0.

- **Core:** ONE reversible additive-coupling recurrent block, tied across the sampled depth K. `model_dim=768`, 12 query / 4 KV heads (GQA), `head_dim=64`, vocab 1024, `seq_len=512`, tied embeddings (token embedding = MoS output basis). Readout `final_norm(z_K)` (RMSNorm of the reversible midpoint) → MoS head, applied OUTSIDE the recurrence so reconstruction stays exact. Optional orthogonal/mHC reversible damping `--damping` (`none` default = `Q=I`, byte-identical; `orthogonal` = `a_{k+1}=Q·a_k+F(...)`, `b_{k+1}=Q·b_k+G(...)` with `Q=expm(S)` the matrix-exponential of a shared low-rank skew, `--damping-rank=16`; exact `Qᵀ=expm(-S)` inverse, applied in the fp64 stream so `recon_rel≈0` stably across training).
- **Reversibility / memory:** O(1)-in-K reversible BPTT (`RevRecurrenceFn` reconstructs activations from the terminal state; no per-depth storage). `recurrence_accum_dtype=float64` carries the coupling stream so the additive ± is exact and gradients are correct under bf16 autocast (`recon_rel≈0`; the backward replays the forward autocast). F/G matmuls run bf16.
- **Attention (MLA):** per-step Multi-head Latent Attention, low-rank KV (`kv_latent=256`), decoupled RoPE, GQA. Single MLA by default; `--attn-moe` makes attention a MoEUT-style MoE over `--n-attn-experts=4` low-rank MLA experts.
- **Experts (MoE — mandatory for effective depth):** `n_experts=16`, `expert_rank=64`, full-D LoRA-style, SOFT DENSE routing (all experts all tokens — no top-k/argmax; reversibility-safe). `router_type=softmax` default (`relu`=ReMoE adaptive sparsity available). `num_shared_experts=0`; `expert_b_init=small` (non-zero `w_out` so the router gets task gradient). `block_order=attn_ffn`, `n_sublayers=1`.
- **MoS head:** `n_mix=2` mixture-of-softmaxes (returns log-probs → `nll_loss`).
...
- **Anti-collapse axes (default OFF; under active study, NOT promoted):** `init_state=x0` default (`random` = Huginn path-independent init, the Occam-first autonomous fix); `step_conditioning=False` (the `M_clk` clock = ceiling diagnostic only, depth-conditioned → NOT generalizable → last resort); `damping=none` default. The autonomous expressiveness rung (rung-2) — an orthogonal/mHC strictly-invertible damping matrix `Q` (relax `A+B=I`: orthogonal `A=Q`, free `B=F/G`; see `opg_doc.tex` §Autonomous substitute) — is **IMPLEMENTED** behind `--damping orthogonal` (low-rank-skew `Q=expm(S)`, `S=UVᵀ-VUᵀ`, projected-subspace apply via thin QR + small `matrix_exp` — exactly orthogonal for any `‖S‖`, no `(I+S)^{-1}` solve, `--damping-rank=16`; norm-preserving so the state traverses rep space and `route_step_div` rises; exact `Qᵀ=expm(-S)` inverse with no Parcae floor, fp64-applied → `recon_rel≈0` stably across training, unlike the Cayley solve which became ill-conditioned as `U,V` grew). Default OFF (byte-identical). **EMPIRICALLY SHELVED (negative result 2026-06):** across two clean attempts orthogonal damping did NOT raise `route_step_div` (MoE basis stayed unengaged ~0.04) AND destabilized the bf16 reconstruction (`recon_rel`→0.85 from F/G state dynamic-range, would need state-norm control). Conclusion: depth-utility comes from the recurrence DYNAMICS (additive coupling + correct fp64 gradients), NOT MoE-basis exponential paths; `route_step_div`/velocity-routing/damping all explored, none drove across-step diversity. MoE is the param-EFFICIENCY (φ_isodepth) basis, not the depth mechanism. See `opg_doc.tex` §Autonomous substitute.
...
- **Two-goal metrics (all emitted; ALL must improve — gate before any full run):** expressiveness — `expressiveness_rho` / I_V(K) (PRINCIPLED headline, below), `val_bpb`, depth-gain `G_T`, `erank`, `disp_tail`; resource — `peak_vram`, VRAM-vs-batch slope, `R_act(K)`, `kv_bytes`; correctness — `recon_rel` (reversible round-trip = gradient-correctness gate). `ops:tok_per_s`/`ops:vram_util_pct` are tuning diagnostics, not gates. Track expressiveness-vs-K at VALIDATION (periodically via `--depth-sweep-every`, below) **including extrapolation beyond the trained max K**: the `--k-eval-sweep` should sweep depths past the trained horizon (e.g. train `k_set={32,64}`, sweep `{4,8,16,32,64,128,256}`) to measure test-time depth scaling (does I_V/val_bpb keep improving — Huginn-style — or degrade/over-think past trained K?). See `opg_doc.tex` §Experimental Protocol / §Metrics.
- **Expressiveness headline = depth-resolved V-usable predictive information I_V(K).** The PRINCIPLED expressiveness metric (Xu 2020 arXiv:2002.10689; Ethayarajh 2022 arXiv:2110.08420) is `I_V(z_K→Y) = H_V(Y) − val_loss(K)` (nats; bits = /ln2), where `H_V(Y)` = empirical next-token marginal cross-entropy (`marginal_token_entropy`, a depth-independent constant, counts all-reduced across ranks). The objective is `corr(I_V(K),K) → +1`: depth monotonically adds usable predictive info. The `--k-eval-sweep` emits per-K `usable_info: K=<k> iv_bits:<..> val_loss:<..>` plus end-of-run `expressiveness_rho:<spearman(K,I_V)>` (`spearman_rho`/`usable_information`) and `iv_total_bits`. V-information is the UNIQUE info measure that can be CREATED by computation (Shannon MI can't grow with depth), so it is sufficient-and-necessary for usable predictive expressiveness w.r.t. readout V; `depth_gain_GT`/`phi_eval` are now SUMMARIES/PROXIES of the same loss-vs-depth curve. **Depth-utility is data-determined; read shallow-vs-deep from the CURVE, not a point:** one `I_V(K)` value can't tell deep from shallow, but the `I_V(K)` curve can — `iv_total_bits = I_V(K_max)−I_V(K_min)` IS the depth-contribution. On easy LM next-token the model gets good largely SHALLOWLY (measured: ~79% of depth-32 usable info already at K=4; depth adds ~0.79 bits), so φ/depth-utility is intrinsically modest on easy LM and HIGH only on depth-demanding data (the `p1_synthetic` depth-hard protocol). A flat I_V curve on easy text is EXPECTED (data-determined), not recurrence collapse — collapse is diagnosed against the positive control. The `--depth-sweep-every N` knob (default 0 = end-of-run only, byte-identical) additionally runs the SAME battery (`depth_sweep`/`depth_gain_GT`/`phi_eval`/`usable_info`/`expressiveness_rho`/`iv_total_bits`) every N validation-aligned steps, step-PREFIXED (`step:<s> depth_gain_GT:<..>`), via `run_depth_sweep(...)`, so `phi_eval`/I_V/`expressiveness_rho`/G_T have a trackable TREND over training (the smoke-gate "all eval metrics improving" needs the depth/expressiveness trends, not just the per-step `metrics:` line and end values); the final unprefixed sweep is always emitted. `experiments/plot_metrics.py` parses the step-prefixed lines into per-step series.
- **Recurrence param-EFFICIENCY φ_isodepth (orthogonal RESOURCE axis, NOT expressiveness) — two metrics, do not conflate.** The PRINCIPLED efficiency metric is the Iso-Depth train-r exponent `phi_isodepth` (`fit_phi_isodepth`): the fitted φ in the scaling law `L(r)=E+A*(N_once+r^φ*N_rec)^-α` over models PRETRAINED at recurrence budgets r∈{1,2,4,8,16}, where `N_once`/`N_rec` = non-recurrent/recurrent (`model.rec`) param counts (`recurrence_param_counts`); φ=1 ⇒ a loop is worth a unique-PARAM block, φ=0 ⇒ worthless, φ>1 ⇒ super-param-equivalent (cap relaxed to [0,3], no longer censored at 1; paper ref ≈0.46). This is the EFFICIENCY axis (how many unique params a loop buys), orthogonal to I_V (the QUALITY axis); both are gated by `recon_rel≈0`. Produced by `experiments/measure_phi.py` (EXPENSIVE multi-train: one model per r, `--k-set <r> --lambda-h 0`); fit is small-sample/noise-fragile with 5 r (use more r/seeds for tight CIs). `phi_eval` (per-run `--k-eval-sweep`, `fit_phi`) is only the CHEAP EVAL-DEPTH PROXY (inference-K log-slope of one trained model) — NOT the Iso-Depth φ.
- **Optimizer / artifact:** Muon (matrix params) + AdamW groups (`build_optimizers`), `beta2=0.95` + grad-clip `1.0` (both match Huginn). LR defaults PROMOTED (HALVED) 2026-06-06: `--matrix-lr 0.01 --embed-lr 0.05 --scalar-lr 0.01` (was `0.02/0.1/0.02`). The old peak LRs DIVERGE on deep-horizon recurrent-depth training (train_loss 6.9→16.5, hidden-state collapse); halving restores monotone train_loss (val_bpb 3.68→2.03, I_V −0.2→+3.7 bits, `expert_util≈2.7`). Huginn-grounded (arXiv:2502.05171 reduced its peak LR for the same recurrent-state collapse; tied weights applied K× ⇒ a single-block LR is ~K× too aggressive). Source of truth is `build_arg_parser` defaults; see `opg_doc.tex` §Training recipe. int6 + zstd scored artifact (`save_int6_artifact` / `load_int6_artifact`).
...
Current 2-GPU DDP assignment (GPUs 6,7 — NEVER `CUDA_VISIBLE_DEVICES=""`):

```bash
CUDA_VISIBLE_DEVICES=6,7 torchrun --standalone --nproc_per_node=2 train_gpt.py --iterations=1000
```
...
- **Audit-test execution.** Every commit touching an M0 optional-component Hyperparameter flag (e.g. `attn_moe`, `damping`, `step_conditioning`, `init_state`, `router_type`), an enforcement-config file, or a symbol in `tests/test_m0_contracts.py::REMOVED_NAMES` MUST run `bash experiments/run_audit_tests.sh` and confirm green BEFORE staging. The canonical registry of audit-test files is the line below, kept inside HTML-comment markers so the parser at `tests/test_audit_test_execution.py` scopes precisely (do not edit the markers or the line between them by hand without also updating `experiments/run_audit_tests.sh::AUDIT_TESTS`): <!-- audit-tests-registry-begin --> `pytest tests/test_m0_contracts.py tests/test_m0_hot_path_sync.py tests/test_enforcement_config_staged.py tests/test_audit_test_execution.py` <!-- audit-tests-registry-end --> . Enforced by `tests/test_audit_test_execution.py`. See [#audit-test-execution](EXPERIENCE.md#audit-test-execution).
...
- **Flag-to-effect contract.** Every M0 optional Hyperparameter flag MUST have an observable, asserted effect on the training-path tensor flow OR scored artifact, exercised by a flip-vs-default test. Enforced per-flag by `tests/test_m0_*.py` (e.g. `test_attn_moe_changes_block_output_vs_single_mla`, `test_damping_none_is_byte_identical_to_additive_coupling`). See [#flag-to-effect-contract](EXPERIENCE.md#flag-to-effect-contract).
- **Removal-symmetry sweep** — removing a Hyperparameter / loss / diagnostic is a same-commit sweep across code, `reports/opg_doc.tex`, docstrings, prescriptions, and tests. Enforced by `tests/test_m0_contracts.py` (the `REMOVED_NAMES` ledger, scanning `train_gpt.py` + `reports/opg_doc.tex`). See [#removal-symmetry-sweep](EXPERIENCE.md#removal-symmetry-sweep).
