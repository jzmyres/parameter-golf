...
- `lip_ub_F` (gate-aligned contraction object on the actual two-state Parcae cycle) MUST be reported on EVERY FP eval — both the train-time fast-val emission and every K-sweep row, never gated by `fp_lip_fast_val_every` or `EvalProfile.lip_probe_set`. `lip_ub_T` and `lip_ub_S` (decomposition diagnostics) remain on the cadence because they cost extra JVPs without changing the gate decision. A profile knob silently dropping the gate evidence is the "decision based on metric we haven't measured" failure mode the audit checklist forbids. Enforced by `experiments/test_lip_ub_fix.py::test_lip_ub_F_is_always_probed_on_FP_eval_unconditional_of_cadence_knob`. User directive 2026-05-13.
...
- **Interpret DEQ diagnostics by definition.** Under TBPTT, `deq_recon_err` measures forward distance travelled, not true reconstruction error. The principled FP-convergence signals are: ``rho_F = |λ_max(J_F)|`` (necessary AND sufficient for asymptotic local contraction by Hartman–Grobman; architecture-agnostic), and ``iter_conv_rel < ε`` at deepest K (empirical convergence). ``lip_ub_F = σ_max(J_F)`` is an over-restrictive operator-norm proxy — sufficient but not necessary; iter152 baseline has ``σ_max ≈ 17`` yet converges fine because ``ρ << σ_max`` for non-symmetric ``J``. Use K-sweep plateau and full-BPTT smoke for true reconstruction sanity. Details: [`EXPERIENCE.md#deq-recon-err-interpretation`](EXPERIENCE.md#deq-recon-err-interpretation).
- **Do not misattribute Parcae contraction fixes.** In the active Parcae path, scalar `deq_beta` is fallback-only; the solver uses per-dim `beta = 1 - A_bar`. The `lip_ub_T` probe measures the transition map `T_theta`, not the iterated map, so a `lip_ub_T` reading does not directly characterize convergence — check `rho_F` (spectral radius on the actual two-state cycle F) for the principled signal.
