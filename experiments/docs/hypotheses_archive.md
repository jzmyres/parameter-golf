...
- **Active proposed runs (queue):** iter 143 (`deq_bptt_k=4`); iter 103 / H77 chained N-stage routing (plan only — see `experiments/docs/iter103_chained_routing_plan.md`); iter 144 (IFT adjoint, gated on iter 143 outcome).
...
- **Closed in 2026-05-12 (iter155 cleanup):**
  - **iter 103a/b/c/e/f/d chained routing — all variants closed.** Best chained variant (103a `split_2stage`) was strictly worse than non-chained iter142b on BPB (1.5489 fast / 1.5880 roundtrip vs 142b 1.5112 post-int6) AND step time (30.00 s/step vs 23.46 s/step). 103b (attn-first) regressed further; 103c (mlp-first) OOM'd at step 810 with worse step-800 BPB (1.6308 vs 103a 1.5965). 103e/103f/103d never ran. No principled path to recovery without first improving the non-chained baseline beyond what chains can match. Re-open only if a fundamentally new chaining mechanism emerges. Was Deferred → Closed.
  - **iter 138/140/141 legacy gram queue closed.** Old routing-gram ablations stale under the iter142-refactor flat objective.
  - **H97/H98/H105 closed.** Attn-gate quantization / hard sparse head gate / weak stale proposals — mismatched to active architecture.
  - **iter155 contraction redirection promoted to active queue (commit pending).** New `lyapunov_target` flag; K-sweep emits `lip_ub_T` + `lip_ub_S`; first draft post-int contraction gate triggered on `lip_ub_S` (Parcae single-state convex-blend surrogate $S=\bar A z+(1-\bar A)T_\theta$). `lip_ub` (formerly the only contraction metric, probing $T_\theta$) renamed to `lip_ub_T` and demoted to decomposition diagnostic. iter156 hard $\bar A$ projection governor / new $c\cdot\Delta$ gain queued contingent on iter155 results.
  - **iter155 corrected after user feedback (2026-05-13).** First-draft `lip_ub_S` is NOT the iterated map: the actual Parcae solver iterates a two-state cycle $F(y,z)=(\bar A y+\beta T(z),\;\bar A z+\beta T(\bar A y+\beta T(z)))$ with $\beta=1-\bar A$ (see `RevDEQFunction.forward:4715-4727`).  Single-state $S$ also cannot rescue an expansive transition: $\sigma_{\max}(J_S) \le \bar A + (1-\bar A)\sigma_{\max}(J_T) > 1$ for any $\bar A < 1$ when $\sigma_{\max}(J_T) > 1$.  Corrected design: third probe `lip_ub_F` on the actual cycle (gate-aligned), `lip_ub_S` demoted to advisory surrogate, `lip_ub_T` retained as decomposition.  New `lyapunov_target=iteration_F` value; gate trigger and `fp_bound` formula switched from S to F.  Same commit also fixed the Ā-detach bug in the Lyapunov-on-S/F branches (gradients now flow to Parcae damping as the doc claimed) and corrected the opg_doc.tex sufficiency relations (for the convex blend, $\rho(J_T)<1$ IS sufficient for $\rho(J_S)<1$; damping cannot rescue an expansive T mode in either S or F).
...
These are the high-signal conclusions to consult before proposing new work. Full evidence remains in the archive.

- **Fixed-point robustness:** K-jitter is permanent (H12); beta jitter is permanent (H35); fixed high beta is unsafe (H36); deeper K-floor alone is not automatically better (iter 131 refinement of H12).
- **Routing:** CV/load-balance and per-token sparsity are distinct axes; routing regularizers must operate on combined routed mass (H100); hard Top-K dispatch is RevDEQ-incompatible (H101).
- **Experts:** full-D LoRA-style experts are the current scaling standard (H71); bottleneck experts are closed as a forward scaling path (H69/H70).
- **Regularization:** Lyapunov and HyDRA denoising are disabled under current Parcae-style damping (H67/H68); decomposed routing/diversity losses are the current canonical direction pending iter 142b/142c.
- **Disabled/default-off paths:** CTP, FSQ, BigramHash, sparse attention replacements, and archived component prototypes remain available only as explicit ablations or future heavy work.

## Active Queue and Recent Results
...
3. **Smoke test before launch** (per `CLAUDE.md` Research Protocol): `python experiments/smoke_test.py` with `--routing-gram-output=1` flag — verify loss decreases, no NaN, recon_err normal. The new loss is just `(p_flat.T @ p_flat / N - I/E).pow(2).sum()` — well-behaved arithmetic.
...
**Code changes required** (hyperparameter fan-out rule; see `CLAUDE.md` Audit Checklist and `EXPERIENCE.md#hyperparameter-fanout`):
...
4. Documentation mirror: document the new flag and its semantic meaning in `EXPERIENCE.md#current-architecture-reference` if it belongs in the orientation mirror.
...
4. Documentation mirror + `opg_doc.tex` parameter table, if the knob belongs in the orientation mirror
...
**Verdict path**: val_bpb int6 ≤ 1.4930 + 0.03 = 1.4960 (per `CLAUDE.md` Promotion Rules) AND step_avg ≤ 22.7s (no throughput regression) AND pertoken_entropy < 2.5 (sparsity actually achieved, not just attempted) → promote.
...
- Partial RoPE 16/64 dims → already split per `CLAUDE.md` Architecture Principles
...
default config — strict-gen claim per `CLAUDE.md` Promotion Rules.
...
`experiments/docs/iter103_chained_routing_plan.md`. Prior in-session
...
6. **Test sequence**:
   ...
   - Full 1000-step run + K-sweep matrix + roundtrip int6 per CLAUDE.md §7 protocol mandate (the FULL `k_sweep_table:` matrix copied verbatim into `experiments/docs/hypotheses.md` after).
...
**Algorithm (3-stage pipeline, see `experiments/components/archive/rr_attention.py`):**
...
`experiments/components/archive/rr_attention.py`. Smoke tests PASS:
...
Iter 117b-1 NOT PROMOTED 2026-04-30 (H87b RESULT) — config bumps reverted. Baseline remains iter 117 v5. Structural improvements kept: `router_reg_loss` group, DRY `_run_hutchinson_F` helper (now emitting hutch_F + rd_step per-K in K-sweep), iter 117b-2 X1 Triton entmax kernel + custom_op (default OFF), iter 117b-3 X2 sparse MoE dispatch helper + MLP-path wiring (default OFF), iter 103 X3 chained-routing flag + safety guard (default OFF), archived `experiments/components/archive/` prototypes (chained_block, orthogonal_expansion_routing, rr_attention, sparse_attention_dispatch).
...
6. **Iter 117b-3b** — **THROUGHPUT-BEARING.** Per-expert sparse-Q attention (asymmetric analog of MLP sparse dispatch). Component at `experiments/components/archive/sparse_attention_dispatch.py` PASSED 7/7 smoke. Saves Q + SDPA + Wo per-expert via capacity-padded gather/dispatch; K, V remain dense AND the SDPA call itself is preserved (smaller Q rows but same FA fusion). Net win iff entmax sparsity > gather overhead.
...
10. **Iter 103 (H77, X3)** — Chained 2-stage routing. Flag + safety guard exists (commits `2ad0a13`, `f8fb068`). Block refactor (two routers, halved expert sets, chain logic in `Block.forward`) pending — design scaffold archived at `experiments/components/archive/chained_block.py`.
...
- ~~**Iter 109 (H80)**~~ — K-jitter {10, 16}. **DROPPED 2026-05-02** — testing K < 16 is reverse direction now that iter 118a kernel makes K=32+ throughput-feasible. See iter 131 above for the actual next K iter (doubling min K to 32).
