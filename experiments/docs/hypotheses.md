...
| **1** | **iter163c: TBPTT-aligned anchor-on-deepest consistency loss** *(NEW 2026-05-15, supersedes iter163 extension AND subsumes the planned iter166 "anchor-on-deepest-K")* | Extend `z_stack` with the last `deq_bptt_k - 1` TBPTT-window iterations (so per-iter anchors fire at every K_sampled including K=16). Compute z_{K+1} = F(z_K, x0) via ONE no-grad Parcae iter (~+1 % step time). Loss: `L = anchor_coef · mean_i ‖z_i − z_{K+1}.detach()‖²` for every z_i in z_stack (coarse prefix anchors + fine TBPTT-window). Anchor-on-deepest target z_{K+1} is strictly stronger than pair-with-next (z_{i+1}): signal magnitude is "distance from FP" not just one-step residual, AND there is no trivial-zero collapse risk because target is input-driven (z_{K+1} = F(z_K, x0), not constant). **Delete `_consistency_extend_no_grad` and the extension-coef knob entirely.** | Yes per most-principled-simplest-general directive. **Principled**: anchor-on-deepest pulls every gradient-carrying z toward the model's best FP estimate (most-converged available); the FP equation z = F(z) is the universal asymptotic condition; no trivial-FP risk because target is input-driven. **Simplest**: extend an existing tensor by 2-3 entries + one inline no-grad iter (~10 lines total); delete the entire `_consistency_extend_no_grad` machinery (~30 LOC removal). **General**: any iterative model has this structure (z_{K+1} target works for any DEQ solver, recurrent layer, refinement loop). Trade-off: |z_stack| pair contributions per step (each at a different depth) vs iter163's 1 boundary pair at Δ=K_train. | ~1.05-1.10× iter152 step time (32-35 s/step vs iter163's 50.7 s/step = **~35 % step-time reduction**). Memory: K_bptt × B × T × D ≈ 600 MB on 80 GB H100, marginal. One inline no-grad Parcae iter (~+1 % step time) for z_{K+1}. | Run macro: `--multi-k-consistency-anchor-coef=0.1` (default; extension knobs removed). Promotion: full `val_bpb ≤ 1.471598` (iter163 champion) AND K-extrapolation gap (K=128 − K=16) `≤ +0.0004`. Branch outcomes: (a) BPB tie + K-gap held → ship as default, **delete extension code permanently**; (b) BPB regresses by > +0.001 → revert to iter163 hybrid, document signal-vs-cost trade-off; (c) K-gap widens beyond +0.001 → tune anchor_coef higher (0.2) before reverting. |
| **2** | **iter168: rho_F estimator hardening + restore σ_max(J_F) diagnostic + path-spectral** *(NEW 2026-05-15)* | Three deliverables in one cheap commit: (a) **multi-seed power iteration** — run rho_F probe with 4 random init vectors, take median; closes the K=24/32 estimator-artifact issue (iter163 K-sweep shows rho_F=1.04 / 1.15 at K=24/32 with K=128=0.92 — single-seed power iteration on non-symmetric J is undersampled); (b) **restore `lip_ub_F = σ_max(J_F)` as DIAGNOSTIC** (not gate, not penalty, not prescription input) — restores robustness/basin-size visibility lost in 2026-05-15 removal, prerequisite for iter161-QAT-late which depends on weight-perturbation tolerance bounded by σ_max; (c) **path-spectral**: emit rho_F at K=1, K=K_train/2, K=K_train in K-sweep — quantifies whether spectral radius is itself FP-converging along trajectory. | Yes. **Principled**: no single scalar is necessary AND sufficient for "model is operationally good" — rho_F characterizes asymptotic local convergence only; σ_max characterizes per-step contraction, basin size, robustness to weight/input perturbation. These are independent operational properties. Multi-seed averaging is principled estimator hardening. Path-spectral generalizes the single-point rho_F measurement. **Simplest**: probe-only changes, no training-time code touched, no loss term added. **General**: applies to any iterative model. Crucially: the 2026-05-15 σ_max removal correctly eliminated it as a GATE/PENALTY/PRESCRIPTION input, but wrongly eliminated it as a DIAGNOSTIC. This iter restores only the diagnostic role. | <1 % step time (probes are fast-val + K-sweep only). | Test-only iter; no model change. Output: hardened gate suite + restored σ_max diagnostic in K-sweep tables and fast-val log lines. Promotion: passes existing test suite and emits the three new fields cleanly. No BPB gate. **Prerequisite for iter161-QAT-late**. |
| **3** | **iter161-QAT-late: deterministic STE fake-quant on last 20 % of training** *(NEW 2026-05-15, replaces H94 GPTQ+LQER bundle)* | For the last 200 of 1000 training steps, route each `nn.Linear.weight` through a straight-through-estimator (STE) wrapper: forward applies deterministic `round(W/scale) * scale` at int6 (then int4 if int6 succeeds), backward passes gradient through identity. **Deterministic** (same op every forward) → RevDEQ reverse reconstruction works (unlike iter20 random-noise injection refuted 2026-04). Targets the **22 mBPB fast→full gap** directly by training the model to BE quant-robust, not patching it post-hoc with PTQ. | Yes per most-principled-simplest-general directive. **Principled**: root-cause attack. PTQ optimizes `argmin_W̃ ‖W·X − W̃·X‖²` (per-layer reconstruction) with W fixed; QAT optimizes `argmin_W E[L(quant(W), X, y)]` (joint over the quantization-constrained weight space). The latter is what we actually want. LQER specifically is symptom-patching (low-rank SVD correction for GPTQ's per-layer error). **Simplest**: one STE wrapper, one step-count gate; no SVD machinery, no per-layer calibration walks, no two-mechanism bundle. **General**: applies to any architecture; bit-width and which-layers-quantized are knobs. RevDEQ-safe (deterministic forward) — verified by contrast with iter20 (random injection broke reverse reconstruction). Bonus: makes σ_max(J_F) operationally meaningful, since weight perturbation amplification is bounded by σ_max — restored as diagnostic via iter168. | ~1.02× iter163c step time (STE adds one round/scale per linear). Quantization start at step 800 — backward graph unchanged (STE is identity). Memory: zero overhead (no SVD factors). | Run macro: `--qat-late-start-step=800 --qat-late-bits=6` (then `--qat-late-bits=4` if int6 succeeds). Promotion: full `val_bpb ≤ 1.471598` AND artifact ≤ 16 MB. Branch outcomes: (a) int6-QAT succeeds → try int4-QAT; (b) int4-QAT closes gap → ship as default; (c) QAT regresses BPB > +0.005 → keep iter163c, add GPTQ-only PTQ as iter161b fallback. |
...
| **4** | **iter167: Δ-curriculum on anchor-target depth** *(NEW 2026-05-15, runs on iter163c base)* | Schedule the anchor target's effective depth: at training fraction f, target = z at iteration `min(K_sampled + ceil(f · K_max), K_max)`. Early training: anchor to z_{K+1} (current iter163c, cheap); late training: anchor to z_{K + Δ_max} via Δ no-grad iters. Lets the model first learn a coarse iteration map cheaply, then progressively tighten the FP target with deeper extension as training stabilizes. | Conditional. **Principled** as curriculum on iter163c's free knob: matches the empirical observation that early-training z's are far from FP (rho_F starts ~1.30 in iter163, settles to 0.92) — anchoring to a not-yet-converged target wastes signal early. **Simplest** addition: one schedule formula in the no-grad extension iter count. **General**: same scope as iter163c. | ~1.05-1.20× iter152 step time depending on Δ_max. | Promotion: full `val_bpb ≤ iter163c_baseline` AND K-gap `≤ +0.0002`. Only run if iter163c saturates without closing K-gap to ≤ +0.0002. |
