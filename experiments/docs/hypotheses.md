...
| Near queue | **iter163 PROMOTED 2026-05-15 as new champion** at full val_bpb=1.471598 (Δ −0.000222 vs iter152's 1.471820). Multi-K consistency loss is now default-on. **Routing-balance push class remains CLOSED** (3-iter refutation iter158/162/165). Queue restructured 2026-05-15 (PAUSED per user directive). New priorities: (1) **iter163c TBPTT-aligned consistency anchor** — extend `z_stack` with TBPTT-window intermediates so per-iter FP-condition pairs fire at every gradient step; delete the expensive extension forward; ~1.02× iter152 step time (38 % reduction vs iter163). (2) **iter168 rho_F multi-seed + restore σ_max diagnostic + path-spectral** — addresses K=24/32 rho_F estimator artifacts and restores robustness/basin visibility lost in 2026-05-15 removal (σ_max as diagnostic, NOT gate/penalty); prerequisite for iter161-QAT-late. (3) **iter161-QAT-late** (replaces H94 GPTQ+LQER bundle) — deterministic straight-through fake-quant on the last 20 % of training; root-cause attack on the 22 mBPB fast→full gap; RevDEQ-safe (deterministic, unlike iter20 random noise). (4) **iter166 anchor-on-deepest-sampled-K** — tighter anchor target than per-iter pairs; runs on iter163c base. (5) **iter167 Δ-curriculum** — only if iter166 saturates. **iter164 anytime-K routing CLOSED** (principle conflict with iter163). **iter170 spectral norm on T_θ CLOSED** (FAILED General test — architecture-specific). **iter171 H96 per-group compression CLOSED** (artifact at 47 % of cap; active queue only decreases artifact size). If soft consistency saturates with rho_F > 0.9, accept the asymptotic limitation rather than escalate to non-agnostic mechanism. **iter172 IN-FLIGHT 2026-05-17 (full 1000-step run launched 2026-05-16, run-id `iter172_full_1000step_20260516`)** — recursive nearest-neighbor consistency anchor with K=8 prefix anchor (`--deq-prefix-anchor-set "8,16,24,32,64,128"`), iter163-default jitter `(16,24,32,64,96,128)` weighted `(0.50,0.40,0.07,0.03,0.015,0.0075)`, NO extension term, `--fast-val-k-sweep-set "8,128"` diagnostic. **iter172 PROMOTED 2026-05-17 as new champion** at full_full_validation val_bpb=**1.462898** (Δ −0.008700 vs iter163's 1.471598 = **−8.7 mBPB, 39× iter163's 0.22 mBPB margin over iter152**). K-sweep dominates uniformly: K=16=1.4716, K=24=1.4720, K=64=1.4722, K=128=1.4722 (all 8.5-9.5 mBPB better than iter163's K-sweep). rho_F at K=128=0.77 vs iter163's 0.92 (principled FP-convergence dramatically improved). Routing-quality also better: attn_cv=0.312 (Δ −0.028), mlp_ortho=0.231 (Δ −0.020 — better than baseline). Artifact 7.40 MB. The promotion-propagation change: `Hyperparameters.deq_prefix_anchor_set` default flipped from `()` (which falls back to jitter set) to `(8, 16, 24, 32, 64, 128)` — the K=8 shallow anchor (gap=8 to K=16) gives consistency loss a non-trivial pair at K_sampled=16 (~88% of training steps) without collapsing rho_F like iter170/171's K=4 (gap=12) did. |
...
| **4** | **iter167: constant Δ=8 anchor-target depth** *(REDESIGNED 2026-05-16, runs on iter163c base)* | Replace iter163c's single no-grad extension iter (Δ=1) with **constant Δ=8** no-grad Parcae iters for the anchor target: `z_target = F^8(z_K, x0)`. The target becomes substantially more converged (8 iterations of contractive iteration map deeper into FP basin), AND the benefit concentrates on the COMMON K_sampled=16 case (50 % of training steps) where iter163c's Δ=1 target z_17 is barely more converged than z_16 (residual ≈ 1.5 % per step at ρ ≈ 0.85). At K_sampled=16, Δ=8 target is z_24 — meaningfully closer to true z*. At K_sampled=128, Δ=8 target is z_136. **Supersedes** the prior planned Δ-curriculum: constant Δ is simpler (no schedule formula) and the principled bottleneck is target QUALITY at common K, not target depth scheduling. | Yes per most-principled-simplest-general directive. **Principled**: target signal magnitude scales as `1 − ρ^Δ` ≈ `1 − 0.85^8 ≈ 73 %` of FP gap at Δ=8 vs `1 − 0.85 ≈ 15 %` at Δ=1. Concentrates the benefit on the K=16 regime (50 % of training) which has the largest residual gap from FP. **Simplest**: change a constant from `1` to `multi_k_consistency_extension_delta` (default 8) in the no-grad loop; single new hyperparameter. **General**: applies to any iterative model. | ~1.07-1.10× iter152 step time (8 no-grad iters cost ~6-8 s/step at average K_sampled ≈ 24; only the no-grad forward, no backward overhead). | Run macro: `--multi-k-consistency-extension-delta=8` (new knob, default 0 = iter163c v2 Δ=1 behavior). Promotion: full `val_bpb ≤ iter163c_baseline` AND K-gap `≤ +0.0002`. Run after iter163c v2 completes regardless of iter163c v2 outcome — Δ=8 is a principled improvement to the anchor target on its own merits, not contingent on iter163c saturation. |
| **5** | **iter169: extend K-jitter set with K=256 at half-freq** *(NEW 2026-05-16, runs on iter163c or iter167 base)* | Add `K=256` to `deq_k_jitter_set = (16, 24, 32, 64, 96, 128, 256)` with weight = 0.0075/2 = 0.00375 (half the current K=128 freq). Net step-time cost ~+1.4 % (rare deep sample). Tests whether the anchor-on-deepest design (iter163c v2 or iter167) benefits from occasional EVEN-deeper FP-condition targets when K=256 is sampled. | Principled but marginal. **Principled**: deeper K = more iterations of contractive map = closer to true FP. Anchor target at K=256 (= z_{K+1}=z_{257} under iter163c v2, or z_{K+Δ}=z_{264} under iter167) is the model's strongest possible FP estimate. **Simplest**: 1-line K-jitter set change + 1-line weight change. **General**: applies to any K-jittered iterative model. Concern: marginal gain because (a) K=256 fires only 3-4× per 1000 steps; (b) at K=128 the model already passes the gate (rho_F=0.92, iter_conv_rel=0.012); (c) the bigger gap is at COMMON K=16, not rare K=128 — which iter167 addresses more directly. Tested as a follow-up rather than iter163c bundle to keep one-variable test discipline. | ~+1.4 % step time. | Run macro: `--deq-k-jitter-set "16,24,32,64,96,128,256" --deq-k-jitter-weights "0.50,0.40,0.07,0.03,0.015,0.0075,0.00375"`. Promotion: full `val_bpb ≤ base_iter_baseline` AND K-gap `≤ base_K_gap`. Run AFTER iter167 to compose: if both promote, the deepest-K target is z_{264} which is essentially the true FP. |
