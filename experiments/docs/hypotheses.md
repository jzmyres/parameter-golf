...
| Main active question | Health-first rescue after user directive: promotion hard-gates on validation BPB only. **Tier 1+2 redesign (corrected 2026-05-13)** establishes ρ(J_F) < 1 as the necessary AND sufficient gate (Hartman--Grobman); σ_max(J_F) < 1 was over-restrictive (iter152 has σ_max ≈ 17 yet iter_conv_rel ≈ 0.02). Promotion gate: `rho_F < 1` (theoretical) OR `iter_conv_rel < 0.05` at deepest K (empirical). `lip_ub_F` retained as decomposition diagnostic only. **Routing-balance push class closed 2026-05-15 by 3-iter empirical refutation** (iter158/162/165): post-int `attn_min_share` prescription is empirically refuted as a BPB lever at iter152's operating point. Next active question: can the **Final-K-as-GT consistency loss (iter163)** induce natural FP convergence and unlock K-extrapolation BPB? Sparse routing, orthogonality, EMA load balance, liveness retained as soft diagnostic goals (informational, not BPB-actionable). |
| Near queue | **Routing-balance push class CLOSED 2026-05-15** after 3-iter refutation (iter158/162/165). Next priorities: (1) **iter163 multi-K consistency loss (Final-K-as-GT)** — principled architecture-agnostic FP convergence learning, addresses K-extrapolation behavior; (2) **iter161 quantization bundle (H94 GPTQ+LQER + H96 per-group)** — direct attack on the int6 quant tax (~0.01 BPB headroom at iter152); (3) **iter164 anytime-K routing** — speculative architecture change. iter152 conditional prefix-K anchors remain the BPB-winning reference: final full `1.471820`. iter153 (reverse-KL balance) is default-on but BPB-neutral. iter155 (Lyapunov-on-F) and iter156 (hard governor) closed as principled-tier mismatches (target σ_max instead of ρ). Do not retry pure IFT. |
...
| Running | iter163 multi-K consistency loss for natural FP-convergence learning (2026-05-15, hybrid CM-paradigm) | Two principled loss terms (commit `c4bcd84`): (a) recursive anchor `λ_a · Σ_i ‖z_{prefix_i} − z_{prefix_{i+1}}.detach()‖²` reusing iter152 z_stack; (b) extension `λ_e · ‖z_K − z_{K+Δ}.detach()‖²` extending from z by Δ no-grad Parcae two-state iterations. λ_a=λ_e=0.1, Δ=K_train. Targets ρ(J_F)<1 by *learning* FP convergence via the FP equation T(z*)=z* directly, not via refuted soft penalty (iter155 Lyapunov). Architecture-agnostic per CLAUDE.md most-principled-simplest-general directive. | Yes: principled per Hartman-Grobman + arch-agnostic + reuses existing prefix-anchor infrastructure. Avoids iter144 IFT refutation (no IFT gradient — pure self-distillation across iterations). Avoids iter155 Lyapunov refutation (no operator-norm penalty). Distinct from iter146-147 prefix-anchor NTP (which only supervises NTP at each depth, not consistency between depths). | 1000 steps, ~14h train (~49s/step ≈ 1.5× iter152 baseline due to extension forward), peak VRAM ~36 GB. Extension uses `_consistency_extend_no_grad` helper that initializes y=z (exact at FP) and runs Δ no-grad Parcae iterations. | **20-step smoke landmark (commit `c4bcd84`)**: rho_F dropped 1.30 → 0.75 in 20 steps (model LEARNED contraction); consistency_anchor_loss 1.05 → 0.0014 (740× reduction); consistency_ext_loss 1.73 → 0.0003 (5400× reduction); val_bpb 4.22 → 2.99 (normal early trajectory); iter_conv_rel 0.148 → 0.006 (tightly converged at K=16). FIRST project mechanism that *trains* FP convergence rather than enforcing or measuring it. Step time ~49s confirmed. Promotion: full val_bpb ≤ 1.471820 (iter152) AND K-sweep monotone (val_bpb at K=128 ≤ val_bpb at K_train, signals learned-convergence). 1000-step run macro: `--multi-k-consistency-anchor-coef=0.1 --multi-k-consistency-extension-coef=0.1 --multi-k-consistency-extension-delta=0 --run-id=iter163_full_1000step_<DATE>`. |
