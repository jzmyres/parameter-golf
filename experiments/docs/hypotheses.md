...
| Near queue | **iter172 PROMOTED 2026-05-17 as new champion** at full_full_validation val_bpb=**1.462898** (Δ −0.008700 vs iter163's 1.471598 = **−8.7 mBPB, 39× iter163's 0.22 mBPB margin over iter152**). K-sweep dominates uniformly K=16→K=128 by 8.5-9.5 mBPB; rho_F at K=128=0.77 vs iter163's 0.92 (principled FP-convergence dramatically improved). Routing-quality side-effects positive: attn_cv −0.028, mlp_ortho −0.020. Artifact 7.40 MB. Promotion-propagation: `Hyperparameters.deq_prefix_anchor_set` default flipped `()` → `(8, 16, 24, 32, 64, 128)` (K=8 shallow anchor preserves iter163's healthy gap=8 regime). **Queue re-optimized 2026-05-17 by post-iter172 principleness audit**: (1) **iter173 K-jitter weight annealing curriculum** — IN FLIGHT (smoke running 2026-05-17, full run pending smoke). (2) **iter161-QAT-late on iter172 base** — code SHIPPED but NOT yet run as 1000-step; promoted to Pri 2 as highest-ROI principled item (direct attack on measured 26 mBPB fast→full gap). (3) **iter175 E=64 + hard-orthogonal expert W_o via Cayley map** — tests user-stated "expert basis" theory (multi-day arch change). (4) **iter178 expand expert rank** — NEW, principled-simplest-first test of "more per-expert capacity"; gates iter176. (5a) **iter176 per-dim sigmoid gate** — conditional on iter178 promoting (structured variant of capacity scaling). (5b) **iter174 iter173-anneal + K=256** — NEW, subsumes iter169; conditional on iter173 promoting. **CLOSED by principleness audit**: iter163c, iter168, iter166 (subsumed into iter172); iter167 (no base — extension term refuted); iter169 (subsumed into iter174); iter164, iter170 spectral-norm, iter171 H96 (previously closed for arch-specificity/budget). If iter173 or iter161-QAT-late ties or regresses, accept that asymptotic ceiling rather than escalate to non-agnostic mechanisms. |
