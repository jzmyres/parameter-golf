...
| Near queue | **iter163 PROMOTED 2026-05-15 as new champion** at full val_bpb=1.471598 (Δ −0.000222 vs iter152's 1.471820). Multi-K consistency loss is now default-on. **Routing-balance push class remains CLOSED** (3-iter refutation iter158/162/165). Next priorities (PAUSED per user directive 2026-05-15): (1) **iter161 quantization bundle (H94 GPTQ+LQER + H96 per-group)** — direct attack on the int6 quant tax (~0.01 BPB headroom); (2) **iter164 anytime-K routing** — speculative architecture change; (3) follow-up iter163-tuning sweeps (e.g. λ=0.2, Δ=2*K_train) to test if more pressure further tightens K-extrapolation. iter153 (reverse-KL balance) and iter152 prefix anchors remain default-on. iter155 (Lyapunov-on-F) and iter156 (hard governor) closed as principled-tier mismatches (target σ_max instead of ρ). Do not retry pure IFT. |
