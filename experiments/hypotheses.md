...
| **28b-tbptt-k8** | Milder TBPTT k=8 (captures ~83% at observed ratio 0.82). Retry after 28-k4 failed; retain TBPTT code per user direction (reusable via `--deq-bptt-k=N`). | **1.974** @ step 563 | **REVERTED (code disabled via `deq_bptt_k=0`)** | FAIL gate: val_bpb 1.974 > 1.917 (+0.072 vs baseline 1.902). VJP ratio at step 100 ≈ 0.82 → last-8 captures 83%, drops 17% bias. Throughput gain small (only K=16 steps affected, ~8% avg). Run shared GPU with stale k=4 process for first 25min of training — some step_avg noise, but step 500 had clean 5539ms step_avg (~27% faster than baseline). **TBPTT conclusion: decay ratio ≈ 0.82 means even k=12 only drops ~7%, giving negligible throughput gain (~3% avg); abandoning TBPTT for now.** Keep code for future re-visits. |
| **29a-W-identity** | Phase 5e-2 transformation sweep #1 on position P-expert-in (current HEAD): replace `sigmoid(Linear(LN(z_in)))·x0` gate with literal identity `x = z_in + x0` (no gate, no W, no scaling). Tests whether the learnable gate was doing useful work or a free always-on injection is equivalent. | TBD | running | KEEP: (a) val_bpb ≤ 1.917 AND (b) K=128 Δ ≤ 0.5. inj_gate module kept instantiated but unused (weights compat). If PASS with simpler mechanism → big simplicity win. |
