...
K=128 vs best-K (K=17, 1.5008) Δ = +0.0031 — well within 0.5 FP-quality gate. K-sweep table `hutch_F` and `rd_step` all `N/A` (SDPA backend rejection under enable_grad — known iter 100b issue, unfixed in this iter).
...
**Current baseline:** iter 100b (`c5154b3`, val_bpb int6 = 1.4893) — last promoted iter; CV-only equilibrium with annealed soft entropy penalty + decoupled regularizers (`min_share_loss_weight=0.0`, `cv_loss_weight=2.0`, `router_entropy_coef=0.005`, `router_entropy_warmup_delay_frac=0.3`) on top of iter 96's E=16 LoRA-style backbone. Improvement of −0.0010 vs iter 96, K=128 vs best-K Δ=+0.00147 (genuine FP across acyclicity primes K∈{17,37,113}), artifact 7.52 MB (47% of 16 MB budget). H76 documents the result. Groups A-D closed; Group F (LoRA-rank/E joint scaling) opened by iter 96, iter 97 closed E>16 axis (H72), iter 98 H73 OOM'd D=1024 on dev. Architectural-sparsity axis closed at the soft-penalty level by H72+H74+H75 trio + iter 100b reframe (entropy mechanism marginally effective at high coef but val_bpb gain is dominantly CV-driven; specialization not the bottleneck on this task). **Iter 97.6 PERMANENT eval-harness change** (Hutchinson-Frobenius + finite-direction Lipschitz + acyclicity primes K∈{17,37,113}) — Hutchinson/Lipschitz silently caught by try/except in iter 100b run (SDPA backend rejection under enable_grad), still pending real validation; acyclicity primes worked perfectly (3/3 genuine FP confirmations). **Iter 100b PERMANENT eval-harness extension (2026-04-28)**: K-sweep now emits a tabular `k_sweep_table:` row per K with 14 columns — `K val_bpb attn_cv mlp_cv pool_cv attn_min mlp_min attn_ortho mlp_ortho pertoken_ent pool_ent shared_gate hutch_F rd_step iter_conv_rel`. Header row at start, fixed-width columns for grep + visual scanning. Value `N/A` for unavailable fields (e.g., Hutchinson when SDPA rejects). Allows direct comparison of routing health, sparsity, and Lipschitz across K within and across iters. Backwards-compatible — existing `k_sweep:k=N ...` line preserved. Active queue post-iter-100b promotion (priority order, 2026-04-29 post-iter-98b close-out): **iter 98b CLOSED** ✗ (NOT PROMOTED — see H81; D=1024 dev-hardware path closed, val_bpb int6 1.5018 vs 1.4893 + per-wallclock +2.6% slower). **Iter 98c** (D=1024 + `num_refinements=0` — principled retry isolating refinement-cost-vs-D-capacity) AWAITING USER GO/NO-GO 2026-04-29. → **iter 104** (AdaSplash α-entmax attention — STILL DEFERRED at custom_op registration; SIGABRT'd in this session as Fix #2 attempt task #111) → **iter 103** (chained 2-stage routing per user spec 2026-04-28; see H77 for design — major architectural rewrite, ~2 hours dev work) → **iter 107** (attn:mlp expert-count ratio sweep 1:1 vs 2:1 vs 1:2 per user spec 2026-04-28; see H78 — router refactor required) → **iter 102** (α-annealing 1.0→1.5 router sparsity; regularization-only after reframe — already tested in this session per task #98, NOT PROMOTED, closed) → **iter 105** (α-jitter {1.0,1.5,2.0} per-step; conditional on iter 102 — closed since 102 didn't promote with margin) → **iter 97.7** (PROFILE-driven throughput retry, proper chrome-trace) → **iter 108** (forward K reduction 16 → 10 per user spec 2026-04-29; see H79 — TRIVIAL config change) → **iter 109** (K-jitter {10, 16} per user spec 2026-04-29; see H80 — TRIVIAL config change) → **iter 95** (TBPTT efficiency sweep — TRIVIAL config sweep) → **iter 106** (NSA-style 3-branch attention; orthogonal architectural axis). **Iter 97.5b PERMANENT (2026-04-29)**: Hutchinson-Frobenius `hutch_F` is now emitted at every val checkpoint (not just K-sweep) — extends iter 97.6's eval-harness change to track FP spectral contraction across training time. See CLAUDE.md §7. E-scaling past 16 closed (H72). Iter 99b sparse dispatch CANCELLED. Iter 102-old (inference-time top-k) removed.
