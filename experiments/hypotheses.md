...
**Priority reshuffle 2026-05-01 (user directive — iter 108 reframed + reordered):** iter 108 changed from "fast-eval-K only" to "training K AND fast-eval K both lowered to match". New spec: `deq_k_jitter_set = (16, 24) → (10, 24)` AND `deq_k_eval = 16 → 10`. K-sweep matrix UNCHANGED (still 4, 8, 16, 17, 24, 32, 37, 64, 113, 128). Rationale: training K should match fast-eval K so the model isn't asked to extrapolate at eval time; K=10 lower bound preserves H12 K-jitter (paired with K=24). Iter 108 reordered to **execute BEFORE iter 95/95b** so the TBPTT depth tests run on the new training-K landscape (with K=10 backward-coverage at deq_bptt_k=2 is now 20%, vs 8.3% at K=24 — meaningfully different regime, more apples-to-apples vs iter 85 era).
...
2. **Iter 112+122 MERGED (H84+H93)** — Gram-matrix penalty (coef=0.1, 10× the first-pass) + logit softcap (Gemma2-style, =30). Re-launch of interrupted iter 112. Integrated at commit `ec4cb19`. **IN FLIGHT 2026-05-01 21:44.** Launch flags: `--use-orthogonal-expansion-routing=1 --routing-gram-coef=0.1 --logit-softcap=30`. s200 val_bpb fast=2.0160 (under 2.05 promotion band, beat first-pass by 0.008). Final + K-sweep ETA ~03:30 tomorrow.
3. **Iter 108 (H79 reframed)** — **TRAINING+EVAL K REDUCTION (NEW PRIORITY 2026-05-01, REORDERED to before TBPTT iters).** Two coupled changes: `Hyperparameters.deq_k_jitter_set = (16, 24) → (10, 24)` AND `deq_k_eval = 16 → 10`. K-sweep matrix UNCHANGED (preserves diagnostic depth coverage). Throughput upside: 50% of training steps now run at K=10 instead of K=16 → ~25% wallclock reduction on K=10 sample steps × 50% sample probability ≈ **~12% step_avg improvement** estimated. Risk: model under-trains at deeper-K extrapolation; mitigated by K=24 sample (50% of steps) preserving deep-K signal. **Verdict path**: if val_bpb int6 ≤ 1.5422 AND step_avg ≤ baseline × 0.92 (~12% throughput win), promote. K-sweep matrix at s1000 will tell us how K=16 (now interpolated) compares.
4. **Iter 95 (H63 retest)** — **TBPTT DEPTH** (after iter 108 — runs on the new K=(10,24) training landscape). Fixed `deq_bptt_k=2 → 3`. Single-line config: `Hyperparameters.deq_bptt_k = 3`. Backward coverage at K=10: 3/10=30% (vs 2/10=20% pre-95) — meaningfully better gradient-depth ratio than the K=24 case. Tests whether the post-iter-108 K=10 floor + extra TBPTT step closes the gap to iter 85 era's 50-67% coverage. Wallclock cost ~+10%; ROI hypothesis: tighter K=128 Δ + faster ntp descent. **Grad_norm signal**: observed 0.07 at iter 112+122 s280 — well below grad_clip=1.0, suggesting headroom. **Verdict path**: if val_bpb improves and step_avg increase ≤15%, promote.
5. **Iter 95b (NEW 2026-05-01)** — **fp32 TBPTT accumulators (conditional).** Replace fp64 Kahan accumulators in `RevDEQFunction.backward` reconstruction with fp32. Error analysis: with `Ā ≥ 0.1` floor, per-step amplification = 1/Ā = 10×, over `deq_bptt_k=3` reconstruction (post-95) = 1000× cumulative; fp32 unit roundoff 1.2e-7 → bound `1000 × 1.2e-7 = 1.2e-4 × ‖z‖`, still safe with 3 orders of margin against the 1e-1 smoke tolerance. **bf16 NOT VIABLE** — same calc with bf16 unit roundoff 7.8e-3 gives `7.8 × ‖z‖`, blows past tolerance. Throughput upside modest (1-3% wallclock — accumulators are memory-bound add/sub, not compute-bound matmul). **Pre-condition**: chrome trace under iter 117 v5 must show fp64 accumulators in hotspot top-5; otherwise skip. Implementation: 5-10 lines (accumulator dtype change + smoke-test tolerance audit + RevDEQ correctness test).
6. **Iter 117b-2 GPU smoke** — **THROUGHPUT-BEARING.** Launch with `--use-entmax-triton=1 --use-entmax-routing=1`. Triton entmax kernel verified vs deep-spin/entmax at fp32 floor; needs DDP+compile+RevDEQ smoke. Tests fused-kernel correctness under blend.
7. **Iter 117b-3 GPU smoke** — **THROUGHPUT-BEARING.** Launch with `--use-sparse-dispatch=1 --sparse-dispatch-capacity-factor=8 --use-entmax-routing=1`. Sparse MoE dispatch numerically equivalent to dense at C ≥ (1-s)·E; smoke at C=8 should be bit-identical, then sweep capacity down to find break-even. **Strongest throughput win** (skip-zero-experts under entmax sparsity).
8. **Iter 117b-3b** — **THROUGHPUT-BEARING.** Per-expert sparse-Q attention (asymmetric analog of MLP sparse dispatch). Component at `experiments/components/sparse_attention_dispatch.py` PASSED 7/7 smoke. Saves Q + SDPA + Wo per-expert via capacity-padded gather/dispatch; K, V remain dense AND the SDPA call itself is preserved (smaller Q rows but same FA fusion). Net win iff entmax sparsity > gather overhead.
...
10. **Iter 112e (NEW 2026-05-01, conditional)** — `block_ortho_aux_coef = 0.1 → 0` ablation under gram=0.1. **Hypothesis**: gram_coef penalty (`‖G − I/E‖²_F` over routing weights) subsumes block_ortho's role (`relu(max_pairwise_cos(expert_output_means) − 0.20)²`). The latter targets the consequence (output similarity); the former targets the cause (routing correlation). At the iter 117 v5 operating point, block_ortho is already near-dormant (attn_ortho=0.124, mlp_ortho=0.204 below threshold 0.20 → relu gate ≈ 0). Removing it is a CLAUDE.md §8 Simplicity Criterion win IF gram catches the same failure modes. **Pre-condition**: iter 112+122 promotes (confirms gram=0.1 is load-bearing). **Verdict path**: val_bpb int6 ≤ baseline ± 0.01 AND attn/mlp_ortho stay ≤ 0.25 throughout training (no spike that would have engaged block_ortho's relu gate). One-line config, ~0% wallclock impact, principled simplification.
11. **Iter 112f (NEW 2026-05-01, conditional)** — `cv_loss_weight = 2.0 → 0` ablation under gram=0.1. **Hypothesis**: gram diagonal target `G[e,e] = 1/E` subsumes CV's load-balance role. Math: `G[e,e] = (1/N) Σ_t w_t[e]²` — driving this to `1/E` requires balanced load (each expert sees fraction 1/E of mass) AND sharp per-token routing (Cauchy-Schwarz: equality iff `w[e] ∈ {0,1}`). CV penalty is the 1D projection of this; gram captures it natively. **Pre-condition**: iter 112e promotes (confirms gram subsumes block_ortho first; sequential ablation for clean attribution). **Verdict path**: val_bpb int6 ≤ baseline ± 0.01 AND pool_cv stays ≤ 0.7 throughout (no winner-take-all collapse). One-line config; principled.
12. **Iter 112g (NEW 2026-05-01, conditional)** — `router_entropy_coef = 0.005 → 0` ablation under gram=0.1. **Hypothesis**: gram off-diagonal target `G[e,e'] = 0` subsumes entropy reg's per-token specialization role. Math: `G[e,e'] = (1/N) Σ_t w_t[e]·w_t[e']` — driving this to 0 requires per-token routing to be one-hot (or disjoint-supported), which is exactly `H(w_t) → 0`. **Pre-condition**: iter 112f promotes. **Verdict path**: val_bpb int6 ≤ baseline ± 0.01 AND pertoken_entropy descends or holds vs baseline trajectory. Removes entropy-warmup-delay machinery + cold-start trap (H87b/iter 117b-1 history).
13. **Iter 112h (NEW 2026-05-01, conditional)** — joint removal of all three under gram=0.1: `block_ortho_aux_coef=0` + `cv_loss_weight=0` + `router_entropy_coef=0`. **Pre-condition**: 112e/f/g all promote individually. **Hypothesis**: no cross-coupling, gram alone is the single router regularizer. **Verdict path**: val_bpb int6 ≤ baseline ± 0.015 AND all routing-health metrics stable. If passes: massive architectural simplification — collapse 3+ coefs to 1 principled gram target, remove entropy-warmup machinery, remove min_share/CV antagonist debug history.
