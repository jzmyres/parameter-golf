...
10. **Iter 112g (PROMOTED to first ablation 2026-05-02)** — `router_entropy_coef = 0.005 → 0` under gram=0.1. **Hypothesis** (confirmed by gradient analysis): L_ent is ~2% of total routing-reg gradient at current coefs and OPPOSES L_G at concentrated states. Removing it is near-strict-gen. **Pre-condition**: iter 112+122 promotes. **Verdict path**: val_bpb int6 ≤ baseline ± 0.005 AND pertoken_entropy descends or holds. Removes entropy-warmup-delay machinery + cold-start trap (H87b/iter 117b-1 history).
11. **Iter 112e (DEMOTED to second 2026-05-02)** — `block_ortho_aux_coef = 0.1 → 0` ablation under gram=0.1. **Hypothesis revised**: L_BO operates on expert OUTPUT means (orthogonal axis to L_G's routing weights), so they are NOT redundant in theory. But empirically at iter 117 v5 baseline, L_BO is near-dormant (relu gate triggers only above 0.20 threshold). **EXPECTED TO FAIL** under the math, but worth empirical test. **Pre-condition**: iter 112g promotes. **Verdict path**: val_bpb int6 ≤ baseline ± 0.01 AND attn/mlp_ortho stay ≤ 0.25 throughout (no parameter-level collapse).
12. **Iter 112h (REVISED 2026-05-02)** — joint removal of L_ent + L_BO (excludes L_CV): `router_entropy_coef = 0` + `block_ortho_aux_coef = 0`. **Pre-condition**: 112e/g both promote. Tests "gram + CV are the dominant pair" hypothesis. **Verdict path**: val_bpb int6 ≤ baseline ± 0.01 AND routing-health metrics stable.
