...
_USE_SPARSE_ATTN_HEAD_GATE: bool = False
_SPARSE_ATTN_GATE_WINDOW: int = 12
_SPARSE_ATTN_GATE_SCALE: float = 1.0
_SPARSE_ATTN_GATE_FACTOR: float = 2.0


def set_sparse_attn_head_gate_enabled(
    enabled: bool,
    *,
    gate_window: int = 12,
    scale: float = 1.0,
    ...
    global _USE_SPARSE_ATTN_HEAD_GATE, _SPARSE_ATTN_GATE_WINDOW
    global _SPARSE_ATTN_GATE_SCALE, _SPARSE_ATTN_GATE_FACTOR
    _USE_SPARSE_ATTN_HEAD_GATE = bool(enabled)
    _SPARSE_ATTN_GATE_WINDOW = int(gate_window)
    _SPARSE_ATTN_GATE_SCALE = float(scale)
    _SPARSE_ATTN_GATE_FACTOR = float(gate_factor)


def is_sparse_attn_head_gate_enabled() -> bool:
    return _USE_SPARSE_ATTN_HEAD_GATE
