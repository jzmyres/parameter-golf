...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, mlp_mult: float, num_experts: int = 6,
        ...
        self._out_ortho_cos_sim: float | None = None
        self._out_ortho_loss: Tensor | None = None
        # Phase 4.5 22-rm-hidden-post: removed hidden_post_norm.
        # Testing if RMSNorm on per-expert hidden (shape [R]) was doing useful work.
    ...
    def mix_experts(self, x: Tensor, w: Tensor, *, pre_normed: bool = False) -> Tensor:
        ...
        # Phase 4.5 22-rm-hidden-post: back to fused view+multiply (no per-expert post-norm).
        h = h.view(N, E, R) * w_flat.unsqueeze(-1)
