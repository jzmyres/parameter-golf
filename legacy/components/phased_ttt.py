...
_USE_TTT_EVAL: bool = False


def set_ttt_eval_enabled(enabled: bool) -> None:
    """Module-level toggle invoked from `train_gpt.py::main()` at startup."""
    global _USE_TTT_EVAL
    _USE_TTT_EVAL = bool(enabled)


def is_ttt_eval_enabled() -> bool:
    return _USE_TTT_EVAL
...
def run_ttt_component_smoke(device: torch.device | str = "cpu") -> dict[str, float]:
    """Small deterministic exec witness for the default-off TTT component.

    This intentionally exercises the trainable per-document LoRA primitive and
    reset semantics without mutating the base model. Full phased validation is
    a separate integration task because the current model uses expert-bank
    projections rather than records-style dense per-layer Linear modules.
    """
    torch.manual_seed(17)
    dev = torch.device(device)
    lora = BatchedLinearLoRA(bsz=2, in_features=8, out_features=4, rank=2).to(dev)
    x = torch.randn(2, 5, 8, device=dev)
    before = lora(x)
    before_norm = float(before.detach().float().norm().cpu().item())
    target = torch.randn(2, 5, 4, device=dev)
    opt = torch.optim.AdamW(lora.parameters(), lr=1e-3)
    opt.zero_grad(set_to_none=True)
    loss = (lora(x) - target).pow(2).mean()
    loss.backward()
    opt.step()
    after_norm = float(lora(x).detach().float().norm().cpu().item())
    lora.reset()
    reset_norm = float(lora(x).detach().float().norm().cpu().item())
    if before_norm > 1e-7:
        raise RuntimeError(f"TTT LoRA zero-init correction was not zero: {before_norm}")
    if after_norm <= before_norm:
        raise RuntimeError("TTT LoRA optimizer step did not change the correction")
    if reset_norm > 1e-7:
        raise RuntimeError(f"TTT LoRA reset did not zero correction: {reset_norm}")
    return {
