...
**Date:** 2026-05-19 review of iter176 / iter176b cleanup
**Rule in CLAUDE.md:** audit checklist row · Audit-test execution

**What happened.** iter176 (commit `a0873fe`, 2026-05-18) added `use_expert_perdim_gate` to `_OPTIONAL_COMPONENT_FLAGS` and shipped a new test file `experiments/test_iter176_perdim_gate.py` covering the MLP class. The new tests exercised the gate via the *internal* kwarg name `use_perdim_gate=True` (the MLP-class parameter), not the *Hyperparameter-level* flag name `use_expert_perdim_gate=True`. The flag-to-effect contract test at `tests/test_optional_component_flag_contract.py::test_each_flag_has_effect_or_explicit_reject` — which walks every `test_*.py` file's AST looking for `use_expert_perdim_gate=True` as a `kwarg`, attribute assignment, dict literal, or `setattr` — found no witness and failed at HEAD. iter176b (commit `6bb59e0`, 2026-05-19) then made an init-only change on top, also without running the contract test. Both commits landed; iter176b launched a 1000-step training run that completed ~510 steps before the user pivoted the iteration on architectural grounds (orthogonal axis: see [hypotheses.md iter176 CLOSED entry]). The contract-test failure was caught 2026-05-19 only during a deliberate code-review pass that ran the audit suite explicitly.

**Root cause.** The "flag-to-effect contract" audit row exists in CLAUDE.md since 2026-05-12 and its enforcement test exists in `tests/test_optional_component_flag_contract.py` since the same date. The gap is not "missing rule" or "missing test" — it's that NEITHER iter176 NOR iter176b ran the test before staging. The CLAUDE.md "Pre-commit review chain" bullet says `/simplify` + three reviewers; it does NOT name a specific pytest suite to execute. The reviewers can in principle run the tests, but in practice agents (including Claude) and humans both pattern-match "I wrote a test" to "the audit gate is satisfied" without re-running the upstream contract test that the new test was meant to satisfy.

**The rule.** Every commit that touches `_OPTIONAL_COMPONENT_FLAGS`, `_OPTIONAL_COMPONENT_CAPABILITIES`, any `use_X` Hyperparameter, an enforcement-config file (`pytest.ini`, `pyproject.toml`, `.pre-commit-config.yaml`, `experiments/run_audit_tests.sh`), or a symbol listed in `tests/test_removal_symmetry.py::REMOVED_NAMES` MUST run `bash experiments/run_audit_tests.sh` and confirm green BEFORE staging. The script wraps `pytest tests/test_optional_component_flag_contract.py tests/test_removal_symmetry.py tests/test_enforcement_config_staged.py tests/test_audit_test_execution.py`. The "audited tests" are exactly the AST/grep-driven contract tests whose green-light is required for the audit row they enforce to be observed.

**Companion to** [#audit-row-executability](#audit-row-executability) ("ship a witness"), [#audit-row-self-enforcement](#audit-row-self-enforcement) ("the witness covers every sibling"), and the Research-Protocol Pre-commit review chain ("/simplify + three reviewers"). Those say WHAT to write, WHAT to cover, and HOW to review; this rule closes the orthogonal gap "the test must actually EXECUTE green before commit."

**Verification recipe.** Enforced by `tests/test_audit_test_execution.py` which asserts:
1. `experiments/run_audit_tests.sh` exists, is executable (mode 100755 in the git index, not just on disk), and has shebang `#!/usr/bin/env bash` (or `#!/bin/bash`).
2. The audit-test files named in this CLAUDE.md row (between the `audit-tests-registry-begin/end` HTML-comment markers) AND the files named in `experiments/run_audit_tests.sh::AUDIT_TESTS` are SET-EQUAL — adding a new audit test requires updating BOTH surfaces in the same commit; removing one without the other is also caught.
3. The script's registry contains no test file that doesn't exist on disk (no stale references).
4. The registry has no duplicate entries and contains ≥4 audit tests (current floor matches the 4 core contracts).
5. The HTML-comment markers in CLAUDE.md appear AFTER the "Audit-test execution" row header and BEFORE the next audit row (so a future contributor cannot accidentally point the markers at a sibling row).

**Why a wrapping shell script instead of a `.pre-commit-config.yaml` hook?** `.pre-commit-config.yaml` requires every contributor to run `pre-commit install` once; on a system where that has not been done the gate is silently inactive (the failure mode the [#enforcement-config-staging](#enforcement-config-staging) rule already taught us about). A self-contained Bash script that's documented in CLAUDE.md and tested for completeness is grep-able, copy-pasteable into a CI step, and works without setup. Contributors can later add a `pre-commit` hook on top; the script remains the source of truth.

**Cross-references.** Direct companions: [#audit-row-executability](#audit-row-executability) (this rule's "ship a witness" predecessor — witness EXISTS but was unused), [#audit-row-self-enforcement](#audit-row-self-enforcement) (covers WHICH sites a new policy must sweep; this rule covers WHEN the policy is checked), [#enforcement-config-staging](#enforcement-config-staging) (this rule's enforcement script IS an enforcement-config file and is itself staged per that rule). Indirect: [#flag-to-effect-contract](#flag-to-effect-contract) (the contract the iter176 commits violated), [#removal-symmetry-sweep](#removal-symmetry-sweep) (one of the audit tests run by the script).
